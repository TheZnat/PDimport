...
categories = []
with open(DIR+'/product_im.csv', mode='r') as infile:
    reader = csv.reader(infile, delimiter=';')
    for rows in reader:

        if(rows[0] == 'Внешний код'):
            continue
        ...
        pattern = re.compile(rows[9], re.IGNORECASE)
        name = pattern.sub("", rows[2])
        ...
        data = {
            'ID': rows[0],
            'Артикул': rows[1],
            'НоменклатурнаяГруппа': rows[6],
            'Наименование': name,
            'ПолноеНаименование': rows[2],
            'СсылкаНаКартинку': rows[16],
            'Толщина': rows[12],
            'Ссылка': '',
            'Раздел': rows[0],
            'Серия': rows[7],
...
###
workbook = xlsxwriter.Workbook(DIR_ROOT+'/admin/uploads/2.xlsx')
...
for item in categories:
    id = item['ID']
    sku = item['Наименование']


    if sku not in products:
        products[ item['Наименование'] ] = {
            'sku': item['Раздел'],
